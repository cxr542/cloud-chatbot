...
[질문 예상 Q&A]
- 이 영상 내용만으로 답할 수 있는 **짧은 질문 4~6개**를 적고, 각 줄에 ``Q: … / A: …`` 형식으로 한 줄씩만 쓰세요(한글).

[한 줄 요약]
- 위 블록들과 모순되지 않게 1~2문장.

**분량:** 본문만 **1500~2500자**가 되도록 쓰세요(REF_TIME 블록 제외). 잘릴 위험이 있으면 [질문 예상 Q&A] 개수를 줄이세요(6~12개 → 4~6개).
...
def is_video_kb_summarized_content(content: str) -> bool:
    """지식 베이스에 이미 반영된 **정상** 동영상 요약 본문인지 판별합니다.

    ``[동영상 자료]`` 로 시작하는 플레이스홀더·실패 안내는 False 입니다.

    Args:
        content: ``vector.json`` 청크 ``content`` 필드.

    Returns:
        재요약 없이 검색에 쓸 만한 본문이면 True.
    """
    s = (content or "").strip()
    if len(s) < 80:
        return False
    if s.startswith("[동영상 자료]"):
        return False
    return True
    ...
    *,
    ...
    try:
        ...
        gc = genai.types.GenerationConfig(
            max_output_tokens=int(settings.GEMINI_VIDEO_MAX_OUTPUT_TOKENS),
            temperature=0.18,
        )
        empty_retries = int(settings.GEMINI_VIDEO_EMPTY_BODY_RETRIES)
        use_stream = bool(settings.GEMINI_VIDEO_USE_STREAM_FALLBACK)

        def _unary_once(vf_obj: object) -> str:
            """활성 파일에 대해 unary 한 번만 호출합니다."""
            ...
            text, _diag = _gather_gemini_generate_content_text(resp, filename=filename)
            return text

        def _unary_with_empty_retry(vf_obj: object) -> str:
            """unary 1회 + (설정 시) 빈 본문일 때만 1~2회 추가 시도."""
            text = _unary_once(vf_obj).strip()
            if text:
                return text
            for er in range(empty_retries):
                logger.warning(
                    "동영상 unary 빈 본문 재호출 %d/%d (%s)",
                    er + 1,
                    empty_retries,
                    filename,
                )
                time.sleep(2)
                vf_ref = genai.get_file(vf_name)
                text = _unary_once(vf_ref).strip()
                if text:
                    return text
            return ""
        ...
        for attempt_idx in range(unary_attempts):
            vf = genai.get_file(vf_name)
            try:
                summarized = _unary_with_empty_retry(vf).strip()
